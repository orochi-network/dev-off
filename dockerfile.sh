...
nginx)
  ...
  EXPOSE_PORT=$'\nEXPOSE 80'
  # Fix permissions for non-root nginx: create cache dirs, pid file, and disable 'user' directive
  RUNNER_COMMANDS+=("mkdir -p /var/cache/nginx/client_temp /var/cache/nginx/proxy_temp /var/cache/nginx/fastcgi_temp /var/cache/nginx/uwsgi_temp /var/cache/nginx/scgi_temp && chown -R nginx:nginx /var/cache/nginx && touch /run/nginx.pid && chown nginx:nginx /run/nginx.pid && sed -i 's/^user  nginx;/# user  nginx;/' /etc/nginx/nginx.conf")
